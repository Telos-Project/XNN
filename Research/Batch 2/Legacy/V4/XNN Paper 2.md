...
## 10. Toward Heterogeneous Node Types: Dendrites, Glia, and a Factors-Only Substrate

Closing the CV tension did not close the single-cell question — it sharpened it. Reviewing what a point neuron with two dynamics models still cannot represent surfaced a specific, uneven picture, not a uniform "more detail would help" one. Two gaps stand out as more than realism concerns:

- **Dendrites are not a detail — a real pyramidal neuron with active dendritic subunits behaves closer to a small two-layer network than a single weighted sum** (Poirazi & Mel). Each subunit performs its own local nonlinear summation (an NMDA-spike-style coincidence detection) before the soma linearly combines them. A point neuron cannot approximate this; it structurally lacks the layer.
- **Some of the most canonical cortical computations may specifically require mechanisms we have none of.** Persistent working memory in influential attractor models depends on NMDA receptors' slow kinetics to sustain activity over seconds (Wang); a published hypothesis holds that some memory is "activity-silent," stored in short-term synaptic facilitation rather than firing at all (Mongillo, Barak & Tsodyks) — a mechanism distinct from our long-term STDP. Population-level dynamics (balance, criticality) are well studied at our current abstraction level; these specific, named computations may not be reachable at it.

This motivated two proposed new node types, both still design-stage, not yet built: **dendritic subunits**, connected to their parent soma by a link that is deliberately *not* an ordinary synapse (it is a fixed structural fact about what makes a set of subunits and a soma one cell — the same slow structural-plasticity process that prunes idle synapses must never be allowed to touch it, or a neuron could quietly disconnect from its own body); and **glial nodes**, sensing local activity and applying local, spatially-restricted damping — a more principled retry of a much earlier, purely global damping attempt that had failed to help the pre-pivot collapse problem.

Designing both exposed a real schema question: with genuinely heterogeneous node types on the horizon, privileging `weight` and `state` as the universal load-bearing fields no longer made sense — biologically, they are not more fundamental than any other property. **This produced a real, tested refactor, completed in this phase**: `weight` and `state` remain in the data model — never removed, so XNN variants built on more conventional machine-learning conventions stay structurally compatible — but are now purely binary (weight: 1 if a connection exists else 0; state: 1 if a neuron fired this tick else 0). Every quantity OXNN's own dynamics actually depend on moved into `factors.strength` (real synaptic efficacy) and `factors.membranePotential` (real LIF dynamics; Izhikevich's `v`/`u` already lived there). One real bug was caught and fixed in the process: input clamping had written directly to top-level `state`, which firing determination used to read — moving firing to `factors.membranePotential` without redirecting the clamp would have silently disconnected external input from the network entirely. Fixed and verified directly (clamp → confirm the real variable changed → confirm firing follows on the next tick), alongside full reproduction of every prior single-cell and network result with zero regression.

---

## 11. Discussion

**The pivot was the right call, and the evidence for that is cumulative, not a single result.** DishBrain's negative outcome was real and well-earned, but the deeper finding was that it was the wrong question for this branch — nothing beneath it had been validated. Once redirected to single-cell fidelity, every subsequent step produced genuine, checkable progress: an unexpected failure direction, a real methodological trap caught before it could contaminate results, a clean mechanistic explanation for why linear adaptation cannot burst, five real classes reproduced by a richer model, a genuine tension between two desirable properties resolved rather than left as a plausible-sounding fix, and — most recently — an honest accounting of what a point neuron still cannot do, precise enough to motivate a real architectural change rather than a vague sense that "more detail would help."

**Where optimism is earned, and it keeps compounding.** Two of three major single-cell classes were reached with minimal added mechanism; all five tested, including genuine bursting, were reached with a still-abstract, still cheap richer model. The CV tension is substantially closed. And the module's interface is now genuinely ready for heterogeneous node types — not as an aspiration, but as a tested refactor that preserves every prior result while removing the specific schema assumption (privileged `weight`/`state` fields) that would have made dendritic subunits and glial nodes awkward to add cleanly.

**Where it isn't yet.** Dendritic subunits and glial nodes are designed, not built. The distinction drawn in Section 10 — some cortical-style computation may already be within reach at the current abstraction level (population balance, criticality), while other, specifically-named computations may be structurally out of reach without exact mechanisms (NMDA kinetics for persistent memory, short-term plasticity for silent memory, dendritic nonlinearity for richer per-neuron computation) — is itself a hypothesis drawn from the literature, not yet tested against this substrate. DishBrain remains formally unanswered, deferred rather than resolved.
...
- The noise-CV relationship is characterized empirically across one sweep, not derived; the right amplitude must be found per network context rather than computed.
- Firing-rate distribution shape and small-world connectivity structure remain completely unmeasured.
...
- Recalibration has not been re-checked since noise injection was added on top, which itself changes spontaneous dynamics.
- Dendritic subunits and glial nodes are design-stage only; the factors-only refactor that enables them cleanly has been built and tested, but neither node type itself yet exists in the module.
- No synaptic kinetics, short-term plasticity, or NMDA-style voltage-dependent transmission exist at all; per Section 10, these may be more than realism gaps for specific canonical cortical computations, not just missing detail.

---

## 13. Conclusion and Future Work

This phase's real result is not a number — it is the discovery that the right question for OXNN was never "can it learn," but "does it behave like real neural tissue, from the single cell up," and that asking it produces answers that build on each other, including architectural ones. Two items from this phase's own prior future-work list are now done — criticality was re-verified and corrected, noise injection was built and shown to work — and a third, the factors-only refactor, was completed ahead of being asked for, because the single-cell work itself surfaced the need. What follows from that:

1. **Build dendritic subunits and glial nodes**, now that the schema cleanly supports heterogeneous node types — the concrete next architectural step, not just a design.
2. **Characterize the noise-CV relationship properly**, and re-verify criticality once more now that noise injection changes spontaneous dynamics on top of everything it was last checked against.
...
5. Only once the substrate is validated this thoroughly, **return to task learning** — DishBrain included — as originally intended.

The negative result that began this phase and the positive results that followed it are the same story: asking a more honest question produced better answers, and better answers keep producing sharper questions. That is the right note for a computational biology program to end a chapter on.